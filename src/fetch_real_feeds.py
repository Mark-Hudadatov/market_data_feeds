...
- aapl_stooq.csv  (исторические котировки AAPL, источник: Stooq)
- wti_eia.json    (WTI spot daily из EIA v2 API)
...
def fetch_stooq_aapl_csv():
    # Документация: CSV для тикера с дневной частотой
    # Пример: https://stooq.com/q/d/l/?s=aapl.us&i=d
    base_url = "https://stooq.com/q/d/l/"
    params = {"s": "aapl.us", "i": "d"}  # можно добавить d1/d2 при желании
...
def fetch_eia_wti_json(api_key: str):
    # Документация EIA v2: /v2/petroleum/pri/spt/data/
    # RWTC — WTI spot price, daily
    base = "https://api.eia.gov/v2/petroleum/pri/spt/data/"

    # Возьмём недавний диапазон дат (последние ~365 дней), чтобы файл не был огромным
    end = date.today()
    ...
    # Просто сохраняем как пришло (raw JSON)
    with open(out_path, "wb") as f:
        f.write(data)
